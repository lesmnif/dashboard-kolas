  ...
  const generateBatchCode = async (roomName: string, roomId: string) => {
    ...
    try {
      ...
      const { data: existingBatches, error } = await supabase
        .from("batches")
        .select("batch_code, room_id, created_at")
        .like("batch_code", `${roomCode}-${year}-%`);

      console.log(
        `[DEBUG] All batches matching pattern ${roomCode}-${year}-%:`,
        existingBatches
      );
      ...
      console.log(`[DEBUG] Room: ${roomCode}, Year: ${year}`);
      console.log(
        `[DEBUG] Found ${existingBatches?.length || 0} existing batches`
      );
      console.log(
        `[DEBUG] Existing batch codes:`,
        existingBatches?.map((b) => b.batch_code)
      );
      console.log(
        `[DEBUG] Existing numbers set:`,
        Array.from(existingNumbers).sort()
      );
  ...
  const handleAddBatch = async (e: React.FormEvent) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.log(
          `[DEBUG] Attempt ${attempt}: Using batch code ${batchCode}`
        );
